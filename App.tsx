          ...
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-slate-900 p-8 rounded-[48px] border border-slate-800">
                    <h3 className="text-lg font-black uppercase mb-6 tracking-tight">Ledger Breakdown</h3>
                    <div className="space-y-4">
                        <div className="flex justify-between items-center p-4 bg-slate-950 rounded-2xl border border-white/5">
                            <span className="text-xs font-bold text-slate-400 uppercase">Matchday Income</span>
                            <span className="font-mono font-black text-emerald-400">+£{(team.financials.revenue / 1000000).toFixed(2)}M</span>
                        </div>
                        <div className="flex justify-between items-center p-4 bg-slate-950 rounded-2xl border border-white/5 text-rose-400">
                            <span className="text-xs font-bold text-slate-400 uppercase">Operational Costs</span>
                            <span className="font-mono font-black">-£{(team.financials.expenditure / 1000000).toFixed(2)}M</span>
                        </div>
                    </div>
                </div>
            </div>
              ...
              <div className="bg-slate-900 p-10 rounded-[56px] border border-slate-800 shadow-2xl mb-8">
                  <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-8 text-center">
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className="text-[10px] font-black uppercase text-slate-500 bg-slate-950 px-4 py-1.5 rounded-full border border-white/5">Regular Season Match</div>
                    </div>
                    <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in slide-in-from-bottom-8 duration-700">
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-slate-950/80 p-8 rounded-[40px] border border-blue-500/20 shadow-inner">
                                <h3 className="text-xs font-black uppercase text-blue-400 mb-4 tracking-widest flex items-center gap-2">
                                    <i className="fas fa-clipboard-list"></i> Tactical Breakdown
                                </h3>
                                <p className="text-lg font-bold text-slate-100 leading-relaxed mb-6 italic">
                                    "{lastMatch.tacticalAnalysis}"
                                </p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="bg-blue-600/5 p-6 rounded-3xl border border-blue-500/10">
                                        <h4 className="text-[10px] font-black uppercase text-slate-500 mb-3 tracking-widest">Match Summary</h4>
                                        <p className="text-xs text-slate-400 leading-loose">{lastMatch.summary}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-gradient-to-br from-amber-500/20 to-amber-900/10 p-8 rounded-[40px] border border-amber-500/30 relative overflow-hidden group">
                                <div className="absolute -right-4 -top-4 text-8xl text-amber-500/10 transform rotate-12 transition-transform group-hover:scale-110">
                                    <i className="fas fa-crown"></i>
                                </div>
                                <h3 className="text-[10px] font-black uppercase text-amber-400 mb-4 tracking-widest flex items-center gap-2">
                                    <i className="fas fa-star"></i> Man of the Match
                                </h3>
                                <p className="text-2xl font-black text-white mb-2">{lastMatch.manOfTheMatch.name}</p>
                                <p className="text-[10px] font-bold text-amber-200/60 leading-relaxed uppercase tracking-tight">
                                    {lastMatch.manOfTheMatch.reason}
                                </p>
                            </div>

                            <div className="bg-slate-950 p-6 rounded-[32px] border border-white/5">
                                <h3 className="text-[10px] font-black uppercase text-slate-500 mb-4 tracking-widest">Match Log</h3>
                                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar flex flex-col-reverse">
                                    {matchEvents.map((e, i) => (
                                        <div key={i} className={`p-3 rounded-xl border flex items-center gap-3 bg-white/[0.02] border-white/5 ${e.type === 'GOAL' ? 'bg-emerald-500/10 border-emerald-500/20' : ''}`}>
                                            <span className="font-black text-[9px] text-blue-400 w-6">{e.minute}'</span>
                                            <i className={`fas ${getEventIcon(e.type)} text-xs w-4 text-center`}></i>
                                            <span className="font-bold text-[9px] uppercase tracking-tight truncate flex-1">{e.description}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                  ...
                  {simulating && (
                    <div className="bg-slate-950 p-8 rounded-[40px] border border-white/5 h-[450px] overflow-y-auto flex flex-col-reverse gap-4 scroll-smooth transition-all">
                        {matchEvents.map((e, i) => (
                        ...
                        {matchEvents.length === 0 && (
                            <div className="h-full flex flex-col items-center justify-center text-slate-600 gap-4 opacity-50">
                                <i className="fas fa-whistle text-4xl animate-bounce"></i>
                                <p className="text-xs font-black uppercase tracking-widest">Waiting for Kick-off...</p>
                            </div>
                        )}
                    </div>
                  )}
